...
import { HTTPS_STATUS_CODES } from '@/enums'
...
export const parseJsonApiError = (error: AxiosError): JsonApiError => {
  ...
  switch (status) {
    case HTTPS_STATUS_CODES.BAD_REQUEST:
      return new errors.BadRequestError(error)
    case HTTPS_STATUS_CODES.UNAUTHORIZED:
    ...
    case HTTPS_STATUS_CODES.CONFLICT:
      return new errors.ConflictError(error)
    case HTTPS_STATUS_CODES.INTERNAL_SERVER_ERROR:
